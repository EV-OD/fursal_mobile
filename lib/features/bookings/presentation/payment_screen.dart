      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow('Time', '${widget.booking.startTime} - ${widget.booking.endTime}'),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isProcessing 
                  ...
                  : const Text('Pay with eSewa', style: TextStyle(fontSize: 18, color: Colors.white)),
              ),
            ),
          ],
        ),
  ...
  Future<void> _payWithEsewa() async {
    ...
    try {
      ...
      if (result.hasData) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Payment Failed: ${result.error}")));
  ...
  Future<void> _verifyTransaction(EsewaPaymentResponse response) async {
    try {
        ...
        final esewaData = {
          ...
          'esewaTransactionCode': widget.booking.id, // Using booking ID as transaction code for now
